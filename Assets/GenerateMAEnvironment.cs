...
public class GenerateMAEnvironment : MonoBehaviour
{
        ...
        for (int i = 0; i < wallCount; i++)
        {
            ...
            if (i == door1Position)
            {
                if (doorData.type.Contains("narrow"))
                {
                    ...
                    AddColliderIfNeeded(wallSegment);
    ...
    void PlaceObstaclesInArea(int areaNumber, GameObject floorContainer)
    {
        ...
        for (int i = 0; i < numberOfObjects; i++)
        {
            if (availableCells.Count > 0)
            {
                ...
                // Add collider if needed
                AddColliderIfNeeded(instantiatedObject);
    ...
    // Adds a collider to the game object if it doesn't already have one
    void AddColliderIfNeeded(GameObject gameObject)
    {
        if (gameObject.GetComponent<Collider>() == null)
        {
            // Add a MeshCollider if the game object has a MeshFilter component
            ...
            {
                // Add a BoxCollider as a default option
